...
const mockEmployeeRequests = [
    // Solicitud 1: Vacaciones Aprobadas
    { 
        id: 101, 
        tipo: 'Vacaciones', 
    ...
    // Solicitud 2: Incapacidad Rechazada
    { 
        id: 102, 
        tipo: 'Incapacidad', 
    ...
    // Solicitud 3: Certificado Laboral Generado
    { 
        id: 103, 
        tipo: 'Certificado', 
    ...
    // Solicitud 4: Préstamo Pendiente
    { 
        id: 104, 
        tipo: 'Préstamo', 
        detalle: 'Monto: $500', 
...
const SolicitudesPortal = () => {
    ...
    const renderContent = () => {
        // 🛑 Estilos y Componentes de los inputs (ahora interactivos o con placeholder claro)
        const baseInputStyle = { 
            ...
            boxSizing: 'border-box', // Asegura que el padding no cambie el ancho
            color: 'black'
        };
            ...
            case 'Certificados':
                return (
                    <div style={{ padding: '20px', border: '1px solid #eee', borderRadius: '4px', backgroundColor: 'white' }}>
                        ...
                        {/* 🛑 Input de texto */}
                        <input type="text" placeholder="Destino del certificado (ej: Banco)" style={baseInputStyle} />
            ...
            case 'Nómina':
                return (
                    <div style={{ padding: '20px', border: '1px solid #eee', borderRadius: '4px', backgroundColor: 'white' }}>
                        ...
                        <input type="text" placeholder="Mes/Rango de Nómina (Ej: Enero 2025)" style={baseInputStyle} />
                        <input type="text" placeholder="Motivo de la solicitud" style={baseInputStyle} />
            ...
            case 'Préstamos':
                return (
                    <div style={{ padding: '20px', border: '1px solid #eee', borderRadius: '4px', backgroundColor: 'white' }}>
                        ...
                        <input type="number" placeholder="Monto: $" style={baseInputStyle} />
                        <textarea placeholder="Motivo/Uso de fondos" rows="4" style={{...baseInputStyle, resize: 'vertical'}} />
            ...
            case 'Incapacidades':
                return (
                    <div style={{ padding: '20px', border: '1px solid #eee', borderRadius: '4px', backgroundColor: 'white' }}>
                        ...
                        <label style={{ display: 'block', margin: '5px 0' }}>Fecha de Inicio y Fin:</label>
                        <input type="text" placeholder="Ej: 01/01/2026 al 05/01/2026" style={baseInputStyle} />
    ...
    return (
        <div style={{ maxWidth: '100%', margin: '0 auto', color: 'black' }}>
            ...
            {/* Contenido de Pestaña Activa (Formularios) */}
            <div style={{ border: '1px solid #ccc', padding: '20px', backgroundColor: 'white', borderRadius: '0 4px 4px 4px' }}>
                {renderContent()}
            </div>
            
            <hr style={{ margin: '40px 0' }} />

            {/* SECCIÓN DE HISTORIAL Y SEGUIMIENTO */}
            <h3 style={{ color: '#2c3e50', marginBottom: '20px' }}>Historial y Seguimiento de Solicitudes</h3>
            ...
            {requests.map(req => (
                <div key={req.id} style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h4>{req.tipo} - {req.detalle}</h4>
                        <span style={{ 
                            padding: '5px 10px', 
                            borderRadius: '4px',
                            fontWeight: 'bold',
                            color: req.estado === 'Aprobado' || req.estado === 'Generado' ? 'white' : req.estado === 'Pendiente' ? 'orange' : 'white',
                            backgroundColor: req.estado === 'Aprobado' || req.estado === 'Generado' ? '#28a745' : req.estado === 'Pendiente' ? '#f8f9fa' : '#dc3545',
                    ...
                    <p>
                        <small>Motivo ingresado: *{req.motivoEmpleado.substring(0, 50)}...*</small>
                    </p>
                    
                    {/* VISUALIZACIÓN DE LA RESPUESTA DEL MANAGER */}
                    {req.estado !== 'Pendiente' && (
                        <div style={{ 
                            marginTop: '10px', 
                            padding: '10px', 
                            backgroundColor: req.estado === 'Aprobado' || req.estado === 'Generado' ? '#e6ffe6' : '#ffe6e6', 
                            borderLeft: `5px solid ${req.estado === 'Aprobado' || req.estado === 'Generado' ? '#28a745' : '#dc3545'}`,
                            borderRadius: '4px'
                        }}>
                            <p style={{ margin: 0 }}>
                            ...
                            <p style={{ margin: '5px 0 0 0', fontStyle: 'italic' }}>
                                "{req.comentarioManager || 'El manager no dejó un comentario específico.'}"
                            </p>
                        </div>
                    )}
                    
                    {/* BOTÓN DE DESCARGA (Solo para Certificados Generados) */}
